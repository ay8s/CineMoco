  ...
  switch (line_code) {
    ...
    case 7:   {
                if ((editing) && 
                  ...
                  //if (com.isMaster() && (com_slave_pos != 0)) {
    ...
    */
    ...
    // direction
    case 23:  {  
                if (editing) ui_change_value_boolean_bit(&motor_status, 3);
    ...
    // camera type
    case 26:  {  
                if (editing) ui_change_value_boolean_bit(&exposure_status, 0);
                
                if (core_is_focus_high_with_shutter()) {
                  
                  ui_get_string( 111, 0); // Nikon   
                  strcpy(value_str, lines[0]); 
                
                } else {
                
                  ui_get_string( 110, 0); // Canon   
                  strcpy(value_str, lines[0]); 
                }
    ...
    case 30:  {  
                  ...
                  if (key == KEY_RIGHT) {
                    ...
                    if       (run_dist <  100)                         stepSize = 1;
                    else if ((run_dist >= 100) && (run_dist < 1000))   stepSize = 10;
                    else                                               stepSize = 100;
                      
                  } else if (key == KEY_LEFT) {
                    
                    if       (run_dist >  1100)                        stepSize = 100;
                    else if ((run_dist >= 110) && (run_dist < 1100))   stepSize = 10;
                    else                                               stepSize = 1;
    ...
    case 54:  {  
                ...
                if ((editing) && 
                  ...
                  /*
                  // if we are in video mode and setup style is now cycle-based, 
                  // do another step to step over cycle-based (which is not allowed here)
                  if ((core_is_mode_video()) && (ui_is_setup_style_cycle())) {
                     ui_change_value_numeric_ubyte(&ui_setup_style, 1, 0, DEF_SETUP_STYLE_COUNT - 1, true); 
                  }
                  */
    ...
    case 66:  {  
                  ...
                  if (com.getSlaveCount() > 0) {                  
                    ...
                    if (key == KEY_RIGHT) {
                      ...
                      if (com_slave_motor_pos < com.getSlaveMotorCount() - 1) {
                        ...
                        if (com_slave_pos == 0) com_save_master_settings();
                        
                        // select the next slave
                        if (com.selectNextSlave()) {
                    ...
                    if (key == KEY_LEFT) {
                     ...
                     if ((com_slave_motor_pos > 0) &&
                       ...
                       // we have the master selected right now
                       if (com_slave_pos == 0) {
                ...
                if  (com_slave_pos == 0) {
                  ...
                  ui_get_string( 154, 0); // Dev  
                  strcpy(value_str, lines[0]);
                  itoa(com.getSlaveID(), temp, 10);        
                  strcat(value_str, temp);
                  strcat(value_str, "/");
                  
                  if (com_slave_motor_pos != 255) {
                    strcat(value_str, "M");
                    itoa(com_slave_motor_pos + 1, temp, 10);
                    strcat(value_str, temp);
                    
                  } else {
                    ui_get_string( 155, 0); // n.a. ("--")  
                    strcat(value_str, lines[0]);
                  }  
    ...
    // video speed           
    case 124: { 
                if (editing) ui_change_value_numeric_ubyte(&motor_video_speed, 1, 1, 100, false);
    ...
    case 132: {
                ...
                if (com.getSlaveCount() > 0) {
                  if (com_is_start_keyframe_set()) ui_get_string( 143, 0); // set [v]!                                    
    ...
    case 133: {
                ...
                // create the output string
                if (com.getSlaveCount() > 0) {
                  if (com_is_end_keyframe_set()) ui_get_string( 143, 0); // set [v]!                                    
                  else                           ui_get_string(  13, 0); // set [ ]! 
                } else {
    ...
    case 156: {
                ...
                if (editing) ui_change_value_boolean_bit(&motor_key_status, 5);  // B0010 0000
                              
                if (motor_key_status & B00100000) {
                  ui_get_string( 4, 0); // enabled    
                } else {
                  ui_get_string( 5, 0); // disabled   
                }
