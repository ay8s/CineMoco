...
#define header_height              10   // height of header in px
  ...
  const byte menu_relation_count = 62; 


byte menu_content_relations[menu_relation_count][2] = { 
    
      {  11,   0 }, {  11,  30 }, {  11,  36 }, {  11,  32 }, {  11,  33 }, {  11,  34 }, {  11,   7 }, {  11,  23 }, {  11,  56 },                                            // status screen advanced (timelapse - run based)
      ...
      {  15,   0 }, {  15, 127 }, {  15, 124 }, {  15,  23 }, {  15, 125 }, {  15, 126 },                                                                                      // status screen advanced (video mode - run based)
      ...
      {  22,  45 }, {  22,  42 }, /*{  22,  47 },*/ {  22,  24 }, {  22, 149 }, {  22,  22 },                                                                                      // settings advanced MoToR
...
void ui_paint_screen(byte pressed_key) {
  ...
  // -----------------------------------
  // paint the ,essage that the progrma is done
  else if (ui_is_program_done_screen()) {
...
void ui_load_menu_strings() {
  ...
  for (int i=0; i<menu_relation_count; i++) {
    ...
    if (menu_content_relations[i][0] == current_menu_code) {
      ...
      // S12-M66
      if ((menu_content_relations[i][0] == 12) &&
      ...
      // S16-M66
      if ((menu_content_relations[i][0] == 16) &&
          (menu_content_relations[i][1] == 66) &&      // device / motor
          (
            (!com.isMaster()) ||
            ((com.isMaster()) && (com.getSlaveCount() == 0))
          )) {
