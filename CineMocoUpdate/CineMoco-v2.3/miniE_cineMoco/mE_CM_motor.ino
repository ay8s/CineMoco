...
boolean motor_is_key_start_set()            { return motor_key_status & B10000000; } 
void motor_set_key_start_flag()             { motor_key_status |= B10000000; } 
void motor_delete_key_start_flag()          { deleteBit(motor_key_status, B10000000); }

boolean motor_is_key_end_set()              { return motor_key_status & B01000000; } 
void motor_set_key_end_flag()               { motor_key_status |= B01000000; } 
void motor_delete_key_end_flag()            { deleteBit(motor_key_status, B01000000); } 

boolean motor_is_bouncing()                 { return motor_key_status & B00100000; } 
...
float motor_getCalibrationValue() {
  ...
  // get the actual stepp size (1, 2, 4)
  int divider = pow(2, motor_step_sizes_index);
  
  // return the calibration value
  return motor_calibrations[motor_calibration_index].stepsPerCm / (float) divider;
...
void motor_jog(byte key, byte ramp_type) {
  ...
  boolean old_dir = motor_status & B00001000;
...
void motor_prepare_values(boolean recalculate_steps, boolean create_ramp) {
  ...
  motor_set_direction(motor_status & B00001000);
...
void motor_set_direction(boolean dir) {
  ...
  if (dir) {
    ...
    setBit(motor_status, B00001000); 
    digitalWrite(DEF_MOTOR_DIR_PIN, HIGH);    
  
  } else {
  
    deleteBit(motor_status, B00001000); 
